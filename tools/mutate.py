...
MUTATIONS = [
    ...
    ("市場の予定をしきい値で落とす", "src/00_config.js",
     ...
     "    include: [],"),
    ("全部消すのに同期範囲しか見ない", "src/14_main.js",
     "    const found = listManagedEvents_(calendarId, from, to);",
     "    const found = listManagedEvents_(calendarId, syncWindow_().start, syncWindow_().end);"),
    ("消したあと、自動実行が動いていることを黙っている", "src/14_main.js",
     "  if (countTriggers_() > 0) {", "  if (false) {"),
